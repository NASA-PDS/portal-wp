...
export const getInstruments = createAsyncThunk(
  ...
  async (_:void, thunkAPI) => {
    ...
    let apiUrl = '/api/search/1/products?q=(product_class eq "Product_Context" and lid like "urn:nasa:pds:context:instrument:*")&limit=9999'
    const config = {
      headers: {
...
const instrumentsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getInstruments.fulfilled, (state, action) => {
      ...
      console.log("Returned Instrument Data:", data);
      ...
      data.forEach( (element:{"summary":object, "properties":object}) => {
        ...
        console.log("Instrument source:", source);
        ...
        instrument[PDS4_INFO_MODEL.CTLI_TYPE_LIST.TYPE] = source[PDS4_INFO_MODEL.CTLI_TYPE_LIST.TYPE] ? source[PDS4_INFO_MODEL.CTLI_TYPE_LIST.TYPE] : [];
